...
export const Rekomen = () => {
    const [Rekomenime, setRekomenime] = useState([]);
    const [isloading,setisloading] = useState(true);
    const getallrekomenime = async() => {
        try {
           const getRekomenime = await axios.get(`https://api.jikan.moe/v4/manga`);
           setRekomenime(getRekomenime.data.data);
        } catch (error) {
           console.log(error)
        }
